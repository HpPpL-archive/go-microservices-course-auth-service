...
import (
	...
	"github.com/HpPpL/microservices_course_auth/internal/repository/auth"
...
func (s *server) Create(ctx context.Context, req *desc.CreateRequest) (*desc.CreateResponse, error) {
	...
	userID, err := s.authRepository.Create(ctx, req.Info)
	if err != nil {
		return nil, err
	}
	//var UserID int64
...
func (s *server) Get(ctx context.Context, req *desc.GetRequest) (*desc.GetResponse, error) {
	...
	user, err := s.authRepository.Get(ctx, req.Id)
	if err != nil {
		return nil, err
	}
...
func main() {
	...
	desc.RegisterAuthV1Server(s, &server{authRepository: auth.NewRepository(pool)})
